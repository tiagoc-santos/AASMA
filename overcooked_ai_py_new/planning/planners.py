...
class JointMotionPlanner(object):
    """A planner that computes optimal plans for multiple agents to
    ...
    def get_low_level_action_plan(self, start_jm_state, goal_jm_state):
        ...
        if self.start_orientations:
            ...
            if any([s == g for s, g in zip(starting_positions, goal_positions)]) or (
                SAFE_RUN and len(set(goal_positions)) != len(goal_positions)
            ):
    ...
    def _populate_all_plans(self):
        ...
        possible_joint_goal_states = list(itertools.product(valid_player_states, repeat=self.num_players))
    ...
    def _get_plans_from_single_planner(self, joint_start_state, joint_goal_state):
        ...
        assert all(
            plan_lengths[i] == len(pos_and_or_paths[i])
            for i in range(self.num_players)
        )
    ...
    def plans_have_conflict(self, joint_start_state, joint_goal_state, pos_and_or_paths, plan_lengths):
        ...
        for t in range(min_length):
            curr_positions = tuple(
                pos_and_or_paths[i][t][0]
                for i in range(self.num_players)
            )
    ...
    def _join_single_agent_action_plans(self, joint_start_state, action_plans, pos_and_or_paths, finishing_time):
        ...
        end_joint_state = tuple(
            pos_and_or_paths[i][finishing_time - 1]
            for i in range(self.num_players)
        )
        joint_action_plan = list(
            zip(*[action_plans[i][:finishing_time] for i in range(self.num_players)])
        )
    ...
    def _handle_path_conflict_with_same_goal(self, joint_start_state, joint_goal_state, action_plans, pos_and_or_paths):
        ...
        solutions = []
        for wait_agent_idx in range(self.num_players):
            solution = self._handle_conflict_with_same_goal_idx(
                joint_start_state,
                joint_goal_state,
                action_plans,
                pos_and_or_paths,
                wait_agent_idx=wait_agent_idx,
            )
            solutions.append(solution)

        valid_solutions = [s for s in solutions if s[0] is not None]
        assert valid_solutions, "No conflict-free plan found"

        best_plan_idx = np.argmin([min(plan_lengths) for _plan, _end, plan_lengths in valid_solutions])
        return valid_solutions[best_plan_idx]
    ...
    def _handle_conflict_with_same_goal_idx(self, joint_start_state, joint_goal_state, action_plans, pos_and_or_paths, wait_agent_idx):
        ...
        plan_lengths = [len(plan) for plan in action_plans]
        prev_positions = [start_pos_and_or[0] for start_pos_and_or in joint_start_state]
        curr_pos_and_or = list(joint_start_state)

        joint_plan = []
        while any(indices[i] < plan_lengths[i] for i in range(num_players)):
            next_pos_and_or = []
            for i in range(num_players):
                if indices[i] < plan_lengths[i]:
                    next_pos_and_or.append(pos_and_or_paths[i][indices[i]])
                else:
                    next_pos_and_or.append(curr_pos_and_or[i])

            next_positions = tuple(pos_or[0] for pos_or in next_pos_and_or)
            curr_joint_action = [Action.STAY] * num_players

            if self.mdp.is_transition_collision(prev_positions, next_positions):
                for i in range(num_players):
                    if i == wait_agent_idx or indices[i] >= plan_lengths[i]:
                        curr_joint_action[i] = Action.STAY
                        next_pos_and_or[i] = curr_pos_and_or[i]
                    else:
                        curr_joint_action[i] = action_plans[i][indices[i]]

                next_positions = tuple(pos_or[0] for pos_or in next_pos_and_or)
                if self.mdp.is_transition_collision(prev_positions, next_positions):
                    return None, None, [np.Inf] * num_players

                for i in range(num_players):
                    if i != wait_agent_idx and indices[i] < plan_lengths[i]:
                        indices[i] += 1
            else:
                for i in range(num_players):
                    if indices[i] < plan_lengths[i]:
            ...
            curr_pos_and_or = list(next_pos_and_or)
            joint_plan.append(curr_joint_action)
            prev_positions = [pos_or[0] for pos_or in curr_pos_and_or]

        end_pos_and_or = tuple(curr_pos_and_or)
        finishing_times = [np.Inf] * num_players
        for i in range(num_players):
            if i != wait_agent_idx:
                finishing_times[i] = indices[i]
    ...
    def _joint_graph_from_grid(self):
        """Creates a graph instance from the mdp instance. Each graph node encodes joint positions."""
        state_decoder = {}
    ...
    def _get_valid_successor_joint_positions(self, starting_positions):
        ...
        joint_motion_actions = itertools.product(Action.MOTION_ACTIONS, repeat=self.num_players)
