...
class OvercookedGridworld(object):
    ...
    def find_free_counters_valid_for_both_players(self, state, mlam):
        """Finds all empty counter locations that are accessible to all players."""
        free_counters = self.get_empty_counter_locations(state)
        free_counters_valid_for_all = []
        ...
        for free_counter in free_counters:
            ...
            if accessible_to_all:
                free_counters_valid_for_all.append(free_counter)

        return free_counters_valid_for_all
    ...
    def is_dish_pickup_useful(self, state, pot_states, player_index=None):
        """
        Useful if:
        - Number of dishes held by players < number of ready/cooking/partially full soups
        - There are no dishes on counters (prevents reward hacking)
        """
    ...
    def is_dish_drop_useful(self, state, pot_states, player_index):
        ...
        any_player_holding_onion = any(
            player.has_object() and player.get_object().name == "onion"
            for player in state.players
        )
        return all_non_full and not any_player_holding_onion

    def is_ingredient_pickup_useful(self, state, pot_states, player_index):
        """
        Always useful unless:
        - All pots are full and no player is holding a dish
        """
        all_pots_full = self.num_pots == len(self.get_full_pots(pot_states))
        any_player_has_dish = any(
            player.has_object() and player.get_object().name == "dish"
            for player in state.players
        )
        return not (all_pots_full and not any_player_has_dish)
    ...
    def is_ingredient_drop_useful(self, state, pot_states, player_index):
        ...
        any_player_holding_dish = any(
            player.has_object() and player.get_object().name == "dish"
            for player in state.players
        )
        return all_pots_full and not any_player_holding_dish
    ...
    def lossless_state_encoding_shape(self):
        ...
        channels = 16 + self.num_players * 5
        return np.array(list(self.shape) + [channels])
    ...
    def lossless_state_encoding(self, overcooked_state, horizon=400, debug=False):
        """Featurizes a OvercookedState object into a stack of boolean masks for CNNs."""
