...
from overcooked_ai_py.agents.agent import AgentPair, AgentGroup, RandomAgent, GreedyHumanModel
...
class AgentEvaluator(object):
    ...
    def evaluate_random_pair(self, num_games=1, all_actions=True, display=False, native_eval=False):
        num_players = self.env.mdp.num_players
        agents = [RandomAgent(all_actions=all_actions) for _ in range(num_players)]
        agent_group = self._build_agent_group(agents)
        return self.evaluate_agent_pair(agent_group, num_games=num_games, display=display, native_eval=native_eval)

    def evaluate_human_model_pair(self, num_games=1, display=False, native_eval=False):
        num_players = self.env.mdp.num_players
        agents = [GreedyHumanModel(self.env.mlam) for _ in range(num_players)]
        agent_group = self._build_agent_group(agents)
        return self.evaluate_agent_pair(agent_group, num_games=num_games, display=display, native_eval=native_eval)

    def _build_agent_group(self, agents, allow_duplicate_agents=False):
        return AgentGroup(*agents, allow_duplicate_agents=allow_duplicate_agents)
            ...
            if isinstance(a0, (list, tuple)):
                ...
                if a1 is not None:
                    if isinstance(a1, (list, tuple)):
                        agents.extend(list(a1))
                    else:
                        agents.append(a1)

                if len(agents) < num_players:
                    agents.extend([agents[-1]] * (num_players - len(agents)))

            if len(agents) != num_players:
                raise ValueError("Expected {} agents, got {}".format(num_players, len(agents)))

            agent_group = self._build_agent_group(agents, allow_duplicate_agents=True)
            trajs = self.evaluate_agent_pair(agent_group, num_games=num_games, game_length=game_length, start_state_fn=start_state_fn, display=display, info=info)
            return trajs, trajs
