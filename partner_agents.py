...
# ---------------------------------------------------------------------------
# Wrapper patch  (add to OvercookedSelfPlayWrapper.step in baseline.py)
# ---------------------------------------------------------------------------
#
# Replace the partner prediction block inside step() with:
#
#   if self.partner_model is not None:
#       partner_obs = self.current_obs_tuple[partner_idx].flatten()
#
#       # --- NEW: pass state to heuristic agents ---
...
#           partner_obs, **predict_kwargs
#       )
#       partner_action_str = Action.INDEX_TO_ACTION[partner_action_idx]
#   else:
#       partner_action_str = Action.STAY
#
# Then add  `needs_state = True`  as a class attribute on every heuristic
# agent above that requires it (GreedyChefAgent, SpecialistAgent, etc.).
#
# ---------------------------------------------------------------------------
# Quick usage example
# ---------------------------------------------------------------------------
...
#     RandomPartner, StationaryPartner, GreedyChefAgent,
#     SpecialistAgent, NoisyGreedyAgent, NoisyPPOPartner,
# )
...
#     NoisyGreedyAgent(epsilon=0.10),
#     NoisyGreedyAgent(epsilon=0.40),
#     NoisyPPOPartner(PPO.load("checkpoint_200k"), epsilon=0.20),
#     NoisyPPOPartner(PPO.load("checkpoint_500k"), epsilon=0.05),
# ]
#
# # In your FCP training loop:
# import random
# env.set_partner_model(random.choice(partner_pool))